...
async function resolveHostId(body) {
  const { host_id, host_name, host_email, host_ad_object_id } = body;
  if (host_id) return host_id;
  if (host_email) return (await findOrCreateHostByEmail(host_name || host_email, host_email, host_ad_object_id)).id;
  if (host_name) return (await findOrCreateManualHost(host_name)).id;
  return null;
}
...
router.get('/', authenticate, async (req, res) => {
  ...
  const rows = await db.prepare(`${SELECT} ${whereClause} ORDER BY p.expected_date ASC, p.expected_time ASC`).all(...params);
  res.json(rows);
});
...
router.post('/', authenticate, async (req, res) => {
  ...
  const hostId = await resolveHostId(req.body);
  ...
  const prereg = await db.prepare(`${SELECT} WHERE p.id = ?`).get(result.lastInsertRowid);
...
router.put('/:id', authenticate, async (req, res) => {
  ...
  res.json(await db.prepare(`${SELECT} WHERE p.id = ?`).get(req.params.id));
});

// POST /:id/checkin — employee checks in a preregistered visitor
router.post('/:id/checkin', authenticate, async (req, res) => {
  const prereg = await db.prepare('SELECT * FROM preregistrations WHERE id = ? AND status = ?').get(req.params.id, 'pending');
  ...
  if (!visitor) {
    const r = await db.prepare('INSERT INTO visitors (first_name, last_name, company) VALUES (?, ?, ?)')
      .run(prereg.visitor_first_name, prereg.visitor_last_name, prereg.visitor_company || null);
    visitor = await db.prepare('SELECT * FROM visitors WHERE id = ?').get(r.lastInsertRowid);
  }
...
router.delete('/:id', authenticate, async (req, res) => {
  const prereg = await db.prepare(`${SELECT} WHERE p.id = ?`).get(req.params.id);
  ...
  if (prereg.status === 'pending') {
    ...
    await db.prepare("UPDATE preregistrations SET status = 'cancelled' WHERE id = ?").run(req.params.id);
  ...
  await db.prepare('DELETE FROM preregistrations WHERE id = ?').run(req.params.id);
