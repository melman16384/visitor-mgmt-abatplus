...
router.get('/', ...adminOnly, async (req, res) => {
  const rows = await db.prepare('SELECT id, name, email, role, active, created_at FROM users WHERE active = true ORDER BY name ASC').all();
  res.json(rows);
});
...
router.post('/', ...adminOnly, async (req, res) => {
  ...
  const result = await db.prepare('INSERT INTO users (name, email, password_hash, role, active) VALUES (?, ?, ?, ?, ?)').run(name, email, hash, role, active !== false);
  const user = await db.prepare('SELECT id, name, email, role, active, created_at FROM users WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/:id', ...adminOnly, async (req, res) => {
  ...
  const conflict = await db.prepare('SELECT id FROM users WHERE email = ? AND id != ?').get(email, req.params.id);
  if (conflict) return res.status(409).json({ error: 'E-Mail bereits vergeben' });

  await db.prepare('UPDATE users SET name = ?, email = ?, role = ?, active = ? WHERE id = ?')
    .run(name, email, role, !!active, req.params.id);
  res.json(await db.prepare('SELECT id, name, email, role, active, created_at FROM users WHERE id = ?').get(req.params.id));
});
...
router.delete('/:id', ...adminOnly, async (req, res) => {
  ...
  const inUse = await db.prepare('SELECT 1 FROM visits WHERE checked_in_by = ? LIMIT 1').get(id);
  if (inUse) {
    await db.prepare('UPDATE users SET active = false WHERE id = ?').run(id);
  } else {
